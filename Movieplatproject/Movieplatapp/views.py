...
from .models import Myrating, MyList
...
def detail(request, movie_id):
    ...
    out = list(Myrating.objects.filter(user=request.user).values())
...
def recommend(request):
    ...
    if current_user_id > new_user:
        movie = Movie.objects.get(id=current_user_id)
...
def add_movie(request):
    if request.method == "POST":
        ...
        Youtube_link = request.URL('youtube_link')
        Movie_Logo = request.FILES('movie_logo')

        movie = Movie(title=Title, description=Description, genre=Genre, release_date=Release_Date, actors=Actors,
                      youtube_link=Youtube_link, movie_logo=Movie_Logo)
        movie.save()
    else:
